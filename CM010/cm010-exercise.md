...
For this assignment, see accompanying slides for more information (cm010 slides)

``` r
suppressPackageStartupMessages(library(tidyverse))
suppressPackageStartupMessages(library(nycflights13))
# Contains data sets (Airport, flights, weather, airlines, planes, etc...)
# All sets are related to each other (relational data)
```
...
``` r
#works the same, dplyr finds matching columns
```
...
``` r
# could not find matches because x1 and x3 dont have any matches so printed NA
# so we can tell left_join which column to take from left and from right
```
...
``` r
#will only print values that are the same between both data sets
```
...
It has over 300,000 rows, will take a long time to run

``` r
flights2 <- flights[1:1000,] %>%    #subset for faster run time
...
``` r
#if you only specify one variable, but there are more common variables (ex. year), then dplyr will make 2 year
#columns
...
``` r
# will only match if you have same column names
```
...
``` r
# only changes name of same columns
# will append whatever is there without matching values properly
# so you dont need to have a common variable
```
...
``` r
p <- tibble(x1 = c("A", "A", "A"), x2 = c(1, 3, 5))
q <- tibble(x1 = c("A", "B", "C"), x3 = c(2, 4, 6))
