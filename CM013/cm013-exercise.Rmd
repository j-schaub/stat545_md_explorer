...
```{r}
plot <- ggplot(mtcars, aes(hp, wt)) + 
    geom_point()
ggsave("Practice Plot.png", plot)
```
...
p_scales + 
    scale_x_log10() +
    scale_colour_continuous(trans="log10") #no scale_color_log10 function
```
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10)    #vectorized command
    ) +
...
p_scales +
    ...
    scale_color_viridis_c(
        ...
        breaks  = 10^(1:10),
        labels  = comma_format(),
        option = "green"   #instead of palette
    ) +
    scale_y_continuous(breaks=10*(1:10))
```
...
#ggsave wont work with plotly
#can use saveWidget (below)
